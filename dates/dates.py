...
if __name__ == "__main__":
    ...
    print(
        f"- Hi, Doc! Today is {_get_delorean_date_format(now)}. What are the minimum and maximum years for the Delorean?"
    )
    ...
    print(
        f"- We could also go to {_get_delorean_date_format(second_dest_date)}... But we need a time, what about {_get_delorean_time_format(second_dest_time)}?"
    )
    # return: - We could also go to 'OCT 21 2015 00:00'... But we need a time, what about '07:28'?

    print(
        f"- I was thinking about go to {_get_delorean_date_format(third_dest)}. What are the difference with {_get_delorean_date_format(second_dest)}"
    )
    # return: - I was thinking about go to 'NOV 12 1955 06:38'. What are the difference with 'OCT 21 2015 07:28'

    print(
        f"- We would be traveling {diff.seconds} seconds! {diff.days} days! {diff_years} years, {diff_months} months, {diff_days} days, {diff_hours} hours and {diff_seconds} seconds!"
    )
