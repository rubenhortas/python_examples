...
class Test(unittest.TestCase):
    def test_shunting_yard(self) -> None:
        infix_expressions = [
        ...
        for expression, expected_result in infix_expressions:
            self.assertEqual(expected_result, shunting_yard(expression))

    def test_rpn_to_infix(self) -> None:
        rpn_expressions = [("1 2 3 * + 4 5 6 * + +", "((1+(2*3))+(4+(5*6)))")]

        for expression, expected_result in rpn_expressions:
            self.assertEqual(expected_result, rpn_to_infix(expression))
